...
module.exports = {
        ...
        try {
            ...
            if (paymentProcessor === "paypal") {
                ...
                const orderParams = {
                    ...
                    currency: "AED",
                    amount: totalAmount,
                    redirect_url: `${data?.SERVER_URL}/api/v1/attractions/orders/ccavenue/capture`,
                    cancel_url: `${data?.SERVER_URL}/api/v1/attractions/orders/ccavenue/capture`,
                    language: "EN",
                };
    ...
    capturePaypalAttractionPayment: async (req, res) => {
        try {
            ...
            let transaction;
            transaction = await B2CTransaction.findOne({
                paymentProcessor: "paypal",
                orderId: attractionOrder?._id,
                status: "pending",
            });
            if (!transaction) {
                transaction = new B2CTransaction({
    ...
    captureCCAvenueAttractionPayment: async (req, res) => {
        try {
            ...
            if (order_status !== "Success") {
                ...
                res.writeHead(301, {
                    // Location: `${data?.REACT_APP_URL}/attractions/orders/${order_id}/cancelled`,
                    Location:
                        attractionOrder.digitalPlatform === "app"
                            ? `${data?.REACT_APP_URL}/app/attraction/${order_id}`
                            : `${data?.REACT_APP_URL}/attraction/failure`,
                });
                res.end();
            } else {
                ...
                res.writeHead(301, {
                    Location:
                        attractionOrder.digitalPlatform === "app"
                            ? `${data?.REACT_APP_URL}/app/attraction/${order_id}`
                            : `${data?.REACT_APP_URL}/attraction/${order_id}`,
                });
                res.end();
            }
        } catch (err) {
